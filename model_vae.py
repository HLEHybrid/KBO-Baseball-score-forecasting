...
# 최대 수 클래스 찾기
    ...
    # 모델 체크포인트 설정 및 조기 종료
    filepath = 'models/kbo_model.{epoch}.keras'
    modelckpt = ModelCheckpoint(filepath=filepath, save_best_only=True)
    earlystopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)

    # Sequential 모델 생성
    model = Sequential()
    model.add(Dense(units=256, kernel_regularizer=L2(1e-4), input_shape=(np.shape(x_train_final)[1],)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    model.add(Dropout(0.5))
    model.add(Dense(units=256, kernel_regularizer=L2(1e-4)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    model.add(Dropout(0.5))
