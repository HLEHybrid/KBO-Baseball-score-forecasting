...
if __name__ == "__main__":
    ...
    # 모델 체크포인트 설정 및 조기 종료
    filepath = 'models/kbo_model_no_aug.{epoch:02d}.keras'
    modelckpt = ModelCheckpoint(filepath=filepath, save_best_only=True)
    earlystopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)

    # Sequential 모델 생성
    model = Sequential()
    model.add(Dense(units=256, input_shape=(np.shape(X_train)[1],)))
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-5), 
                  loss=tf.keras.losses.CategoricalFocalCrossentropy(), metrics=['accuracy'])
